...
abstract class DefinitionInteractor : MviInteractor<DefinitionIntent, DefinitionResult>() {
  ...
  abstract val bookmark: ObservableTransformer<DefinitionIntent.Bookmark, DefinitionResult>
  ...
  override val intentsProcessor = ObservableTransformer<DefinitionIntent, DefinitionResult> {
    it.publish { intent ->
      Observable.merge(
        ...
        intent.ofType(DefinitionIntent.Bookmark::class.java).compose(bookmark)
      )
    }
  }
}